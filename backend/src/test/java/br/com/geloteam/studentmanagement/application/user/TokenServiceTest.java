...
class TokenServiceTest {
    ...
    void shouldGenerateTokenCorrectly() {
        ...
        assertEquals("usuario.teste", claims.getSubject());
    ...
    @Test
    @DisplayName("Should return AuthToken with token value and expiry duration")
    void shouldGenerateAuthToken() {
        when(authentication.getName()).thenReturn("user");
        doReturn(List.of()).when(authentication).getAuthorities();

        AuthToken result = tokenService.generateAuthToken(authentication);
    ...
    @Test
    @DisplayName("Should throw RuntimeException when JWT encoding fails")
